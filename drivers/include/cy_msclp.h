...
*   and configure CAPSENSE&trade; widgets. It can be launched in ModusToolbox
...
*
********************************************************************************
* \subsection group_msclp_scan_modes Scan Modes
********************************************************************************
*
* The MSCLP HW block supports three scan modes:
* 1. Interrupt Driven scan mode requires direct HW block
*    registers configuration and then triggering a single-sensor scan.
*    A scan is triggered by a writing the START_FRAME bit into the FRAME_CMD
*    register and then writing the START_SCAN bit into the SNS_CTL register.
* 2. Autonomous-Scan Multi-Sensor (AS-MS) scanning mode allows scanning
*    a series of sensors called "frame". The interrupt is generated after
*    all sensors scans are complete. Before a scan the configuration of all
*    sensors must be stored into the internal HW block SNS.SENSOR_DATA memory
*    using the Cy_MSCLP_WriteSensorDataArray() function.
*    Scanning is triggered by writing the START_FRAME bit into the FRAME_CMD
*    register.
* 3. Low Power Always-On-Scanning (LP-AOS) scanning mode is very similar
*    to AS-MS mode from the configuration perspective. Scanning is triggered by
*    writing the START_FRAME_AOS command into the WAKEUP_CMD register. The
*    differences from AS-MS mode: availability of the wake up timer (WAKEUP_TIMER),
*    scanning interval (FR_TIMEOUT_INTERVAL) and HW channel engine part, which
*    performs raw count processing and detects a touch. So, in this mode
*    scanning can happen periodically without any interrupt until a touch
*    is detected.
*
* \note
* 1. Sensor-related registers of the HW block must be accessed when MRSS is
*    enabled. Otherwise, it leads to device hard fault. Turning on / off
*    MRSS can be done using the Cy_MSCLP_MrssStart() and Cy_MSCLP_MrssStop()
*    functions when the ENABLED bit is disabled in the CTL register.
* 2. The last sensor configuration in a frame must have the LAST bit of
*    SNS_CTL register set for any of the above modes.
*
...
* 1. MSCv3LP-compatible GPIO sensor pads - a subset of GPIOs that can be controlled
...
* 2. New MSCv3LP GPIO Cmod pads - dedicated pads for the external integration
*    capacitors.
...
* If the MSCLP HW block is configured for operation in Interrupt Driven
...
*    MCU low-power mode transitions. The HW block can operate in different
*    power modes and a transition between these modes is allowed.
* 2. In Interrupt Driven mode, if Deep Sleep power mode is needed
*    the user is responsible for providing any Deep Sleep wake up source.
*
...
* device for the design, CAPSENSE&trade; system and sensor design guidelines,
...
* document and the product-specific CAPSENSE&trade; design guide. It is highly
* recommended to start with these documents. They can be found at www.infineon.com.
...
****************************************************************************//**
*
* Starts the AOS frame processing.
*
* \param base
* The pointer to the MSCLP HW block base address.
*
