...
*
* The primary use case is recovering from a CPU or firmware failure.
...
* However, the \ref group_sysclk_ilo is the only clock source for the WDT,
* so its accuracy should be considered.
* Use the ILO compensation technique to increase accuracy, see \ref Cy_SysClk_IloCompensate.\n
...
* generation. The Watchdog Counter (WDC) can be used to generate
...
* After the ILO is enabled, ensure that the watchdog reset
* is disabled by calling the Cy_WDT_Disable() function. Set the WDT match
